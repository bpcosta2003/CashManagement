/**
 * Versão backend dos insights — duplicada de `src/lib/insights.ts` pra
 * evitar dependência cruzada entre /api (CommonJS) e /src (ESM).
 * **Mantenha em sincronia** com o arquivo do frontend; mudanças aqui
 * devem refletir lá e vice-versa.
 */
...
function calcRowMinimal(r: Row): CalcOut {
  const v = +r.valor || 0;
  const d = Math.min(+r.desconto || 0, v);
  return { forma: r.forma, v: v - d, status: r.status };
}
...
export function computeInsights({ rows, mes, ano, now }: Params): Insight[] {
  ...
  const curCalc = cur.map(calcRowMinimal);
...
/**
 * Conta pendentes do mês — usado como primeira seção do email mesmo
 * quando o insight de "pendentes antigos" não dispara (pendência nova
 * também merece atenção no fim do mês).
 */
export function pendingRows(rows: Row[], mes: number, ano: number): Row[] {
  return rows.filter(
    (r) =>
      r.mes === mes &&
      r.ano === ano &&
